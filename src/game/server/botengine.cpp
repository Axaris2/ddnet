...
void CGraph::ComputeClosestPath()
{
	...
	if(m_pClosestPath)
		free(m_pClosestPath);
	m_pClosestPath = (int *)calloc(m_NumVertices * m_NumVertices, sizeof(int));
	if(!m_pClosestPath)
		return;
	int *Dist = (int *)calloc(m_NumVertices * m_NumVertices, sizeof(int));
	if(!Dist)
		return;
...
void CBotEngine::Init(CTile *pTiles, int Width, int Height)
{
	...
	m_pGrid = (int *)calloc(m_Width * m_Height, sizeof(int));
	if(m_pGrid)
	{
		...
		for(int c = 0; c < MAX_CLIENTS; c++)
		{
			...
			m_aPaths[c].m_pVertices = (vec2 *)calloc(m_aPaths[c].m_MaxSize, sizeof(vec2));
			m_aPaths[c].m_pSnapID = (int *)calloc(m_aPaths[c].m_MaxSize, sizeof(int));
			for(int i = 0; i < m_aPaths[c].m_MaxSize; i++)
...
void CBotEngine::GenerateSegments()
{
	...
	m_pSegments = (CSegment *)calloc(m_SegmentCount, sizeof(CSegment));
...
void CBotEngine::GenerateCorners()
{
	...
	m_pCorners = (vec2 *)calloc(CornerCount, sizeof(vec2));
...
void CBotEngine::GenerateTriangles()
{
	...
	m_Triangulation.m_pTriangles = (CTriangulation::CTriangleData *)calloc(2 * CornerCount, sizeof(CTriangulation::CTriangleData));
	vec2 *Corners = (vec2 *)calloc((CornerCount + 3), sizeof(vec2));
	int m = 0;
	for(int i = 1; i < m_Width - 1; i++)
	{
		for(int j = 1; j < m_Height - 1; j++)
...
void CBotEngine::GenerateGraphFromTriangles()
{
	...
	m_Graph.m_pVertices = (CVertex *)calloc(m_Graph.m_NumVertices, sizeof(CVertex));
	m_Graph.m_pEdges = (CEdge *)calloc(m_Graph.m_NumEdges, sizeof(CEdge));
	for(int i = 0; i < m_Triangulation.m_Size; i++)

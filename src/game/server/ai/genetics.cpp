...
void CGenetics::InitGenomes()
{
	for(int i = 0 ; i < m_NumGenomes ; i++)
		for(int j = 0 ; j < m_Size ; j++)
			m_pGenomes[i].m_pGenome[j] = (int)frandom() % 100;
}
...
void CGenetics::Evolve()
{
	...
	for(int i = 0 ; i < (m_NumGenomes >> 1) ; i++)
	{
		CGenome Genome = m_pGenomes[i];
		for(int j = 0 ; j < m_Size ; j++)
			Genome.m_pGenome[j] = m_pGenomes[(int)frandom() % (m_NumGenomes >> 1) + (m_NumGenomes >> 1)].m_pGenome[j];
	}
	// Mutate
	for(int i = 0 ; i < m_NumGenomes ; i++)
	{
		...
		for(int j = 0 ; j < m_Size ; j++)
			if((int)frandom() % 100 == 1)
				Genome.m_pGenome[j] = (int)frandom()%100;
	}
}

CGenetics::CGenetics(int Size, int NumGenomes)
{
	m_pGenomes = (CGenome *) malloc(NumGenomes * sizeof(CGenome));
	for(int i = 0 ; i < NumGenomes ; i++)
	{
		m_pGenomes[i].m_pGenome = (int *) malloc(Size * sizeof(int));
